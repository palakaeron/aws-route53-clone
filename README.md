...
The simulated Route 53 modules also provide search/filter/pagination where appropriate.
...
SQLite keeps the project lightweight and easy to run locally while still providing real persistent relational storage.
...
Create multiple DNS record types.
Edit a DNS record.
Delete a DNS record.
Search and filter records.
Delete the test Hosted Zone.
Confirm associated records are removed.
Open Traffic Policies.
Open Health Checks.
Open Resolver.
Open Profiles.
Log out.
...
Review the GitHub repository and README.
Repository
